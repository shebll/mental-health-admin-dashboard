...
export function Sidebar() {
  ...
  return (
    <>
      <Menu
        size={30}
        className="cursor-pointer absolute left-[22px] top-[18px] 2xl:hidden z-[2]"
        onClick={() => setToggle((prev) => !prev)}
      />
      <span
        onClick={() => setToggle((prev) => !prev)}
        className={`absolute inset-0 w-screen h-screen bg-black/10 backdrop-blur-sm cursor-pointer 
          ${toggle ? "block" : "hidden"} 2xl:hidden z-[2]`}
      />
      <div
        className={`absolute z-[2] 2xl:relative h-screen transition-all duration-500 ease-in-out shadow-xl bg-secondary px-2 py-10 w-[300px] flex flex-col justify-between ${
          toggle ? "left-[-0%]" : "left-[-100%] "
        } 2xl:left-[0%]`}
      >
        <X
          size={30}
          className="cursor-pointer absolute right-[22px] top-[24px] 2xl:hidden"
          onClick={() => setToggle((prev) => !prev)}
        />
